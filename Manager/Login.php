...
require_once __ROOT__.'/Manager/Scripts/ManagerLogin.php';

$allFields = "yes";
$errorlogin = "";

if (isset($_POST['manlogin'])) { 
    if ($_POST['id'] == "") {
        $allFields = "no";
    }
    if ($_POST['password'] == "") {
    ...
    if ($allFields == "yes") {
        ...
        if ($manLogin == null) {
            ...
            Incorrect Manager ID or Password.	
            </div></b></center>";
        } else if (!($manLogin[0]['role'] == "admin")) {
            $errorlogin = "<center><b><div class='alert alert-danger fade show col-10' role='alert style=font-weight: bold;'>
            Your account is not equipped with Manager privileges. 
            </div></b></center>";
        } else {
            RedirectTo('//localhost/GymSite/Manager/Home.php?id=' . $_POST['id']);
        }
    }  
}?>
...
<div class="container bgColour">
    <main>
        <br>
        <div class="col-12">
            <h1>MiniGym Manager Login Portal</h1>
        </div>
        <hr>
        ...
        <form method="post">
          ...
          <br>
          <div class="form-group col-md-6">
            <button type="submit" class="btn btn-success" name="manlogin">Securely Login</button>
          </div>
		</form>
        </div>
        <br>
